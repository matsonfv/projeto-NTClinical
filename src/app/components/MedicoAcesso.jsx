...
import { MG, chavesAcessoMedico, doctors } from '../data/mockData';
...
// ─── Componente Principal: MedicoAcesso ──────────────────────────────────────
// Tela de login do médico via chave de acesso (ex: MED-OLV-001)
export default function MedicoAcesso() {
  ...
  const [loading, setLoading] = useState(false);
      ...
      // Valida a chave de acesso e redireciona para o portal
    ...
    setTimeout(() => {
      const doctorId = chavesAcessoMedico[chave.trim().toUpperCase()];
      if (!doctorId) {
      ...
      if (!doctor) {
        setErro('Médico não encontrado. Tente novamente.');
        setLoading(false);
        return;
      }
      login({ type: 'doctor', doctorId: doctor.id, doctorName: doctor.name, specialty: doctor.specialty });
      navigate('/medico/portal');
    }, 600);
  };
        ...
        >
          ...
          <div className="mt-6 p-4 rounded-xl" style={{ backgroundColor: isDark ? 'rgba(60,90,62,0.18)' : '#EFF5EF' }}>
            ...
            <div className="space-y-1">
              {Object.entries(chavesAcessoMedico).map(([chave, id]) => {
